...
Return consistent API Gateway proxy responses from controllers and handlers: stable status codes, CORS headers, and a uniform error body shape across all endpoints.
...
`fromDatabaseError(error: unknown): APIGatewayProxyResult | null` maps recognized database and DynamoDB errors to HTTP responses so controllers do not need to inspect raw error shapes. Returns `null` for unrecognized errors so the caller can fall through to a generic 500.
...
|---|---|
| `error.code === DatabaseErrorCode.UNIQUE_VIOLATION` | 400 BadRequest — "conflicts with an existing record" |
| `error.code === DatabaseErrorCode.FOREIGN_KEY_VIOLATION` | 400 BadRequest — "references a record that does not exist" |
| `error.name === "ConditionalCheckFailedException"` | 409 Conflict |
| `error.name === "TransactionCanceledException"` | 409 Conflict |

This is especially useful when repositories use `TransactWriteCommand` with `ConditionExpression` guards — those throw `ConditionalCheckFailedException` or `TransactionCanceledException` on conflict, and `fromDatabaseError` translates them into the right HTTP response without the controller needing to know DynamoDB error names.
...
export class RestResult {
    ...
    static fromDatabaseError(error: unknown): APIGatewayProxyResult | null {
        ...
        const name = typeof error === 'object' && error !== null && 'name' in error ? String((error as any).name) : undefined;
        ...
        if (name === 'ConditionalCheckFailedException' || name === 'TransactionCanceledException') {
            return RestResult.Conflict('Request conflicts with an existing record.');
        }
...
Add the remaining methods (`Unauthorized`, `Forbidden`, `Accepted`, `File`, `MovedPermanently`, `BadGateway`, `GatewayTimeout`) following the same pattern when the feature needs them.
