...
Use this reference when the repository needs a reusable runtime composition layer for shared clients, context, and services.

## Goal

Create one module-scope composition root that initializes long-lived clients, context, and shared services once per Lambda runtime.

## Portable Shape

- a composition module such as `src/app.ts` exports singleton SDK clients, service instances, mapper instances, and `context`
...
export const context = new Context(createRepositoryDependencies());
...
- Add a repository to the shared context when more than one controller or service may need it.
